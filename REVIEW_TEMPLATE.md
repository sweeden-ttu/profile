# Blog Post Review Template

**Reviewer:** [Agent Name]
**Review Date:** [YYYY-MM-DD]
**Draft Version:** v[N]
**Lecture:** [Lecture ID] - [Lecture Title]
**Course:** [CS-5384 / CS-5368]

---

## Overall Assessment

**Quality Rating:** [Excellent / Good / Needs Improvement / Poor]

**Summary:**
[2-3 sentence summary of overall quality, major strengths, and primary concerns]

**Recommendation:** [Approve / Approve with Minor Changes / Needs Major Revision / Reject]

---

## Accuracy Review

### Critical Issues
[Issues that must be fixed before publication]

- **[Issue #1]**
  - **Location:** [Section, paragraph, or line reference]
  - **Description:** [Detailed description of the error]
  - **Impact:** [Why this is critical]
  - **Suggested Fix:** [Specific recommendation]

- **[Issue #2]**
  - **Location:** [Section, paragraph, or line reference]
  - **Description:** [Detailed description of the error]
  - **Impact:** [Why this is critical]
  - **Suggested Fix:** [Specific recommendation]

### Minor Issues
[Issues that should be fixed but are not blocking]

- **[Issue #1]**
  - **Location:** [Section, paragraph, or line reference]
  - **Description:** [Description of the issue]
  - **Suggested Fix:** [Recommendation]

### Verification Notes
- [ ] Cross-referenced definitions with lecture PDFs
- [ ] Verified mathematical formulas
- [ ] Checked examples against source materials
- [ ] Validated step-by-step solutions
- [ ] Confirmed terminology consistency

---

## Completeness Review

### Missing Content
[Topics, concepts, or elements that should be included]

- **[Missing Item #1]**
  - **What's Missing:** [Description]
  - **Where It Should Go:** [Suggested location]
  - **Why It's Important:** [Rationale]
  - **Suggested Content:** [Brief outline]

### Incomplete Sections
[Sections that need expansion]

- **[Section Name]**
  - **What's Missing:** [Description]
  - **Suggested Addition:** [Content outline]

### Coverage Assessment
- [ ] All topics from lesson plan covered
- [ ] All exercises addressed
- [ ] Prerequisites explained or referenced
- [ ] Key concepts fully developed
- [ ] Examples cover different aspects
- [ ] External resources relevant and sufficient

---

## Readability Review

### Structure and Flow
- **Overall Structure:** [Assessment]
- **Section Transitions:** [Assessment]
- **Logical Progression:** [Assessment]

**Issues:**
- **[Issue #1]**
  - **Location:** [Section]
  - **Description:** [Problem]
  - **Suggested Improvement:** [Recommendation]

### Clarity and Explanation
- **Introduction:** [Assessment - Does it set context?]
- **Technical Terms:** [Assessment - Are they defined?]
- **Examples:** [Assessment - Are they clear?]
- **Mathematical Notation:** [Assessment - Is it explained?]

**Issues:**
- **[Issue #1]**
  - **Location:** [Section]
  - **Description:** [Problem]
  - **Suggested Improvement:** [Recommendation]

### Writing Quality
- **Tone:** [Assessment]
- **Sentence Structure:** [Assessment]
- **Paragraph Length:** [Assessment]
- **Word Choice:** [Assessment]

**Issues:**
- **[Issue #1]**
  - **Location:** [Section]
  - **Description:** [Problem]
  - **Suggested Improvement:** [Recommendation]

### Reading Time
- **Target:** 15 minutes
- **Estimated:** [X] minutes
- **Assessment:** [Appropriate / Too Short / Too Long]
- **Recommendation:** [If adjustment needed]

---

## Visual Elements Review

### Diagrams
- **Number of Diagrams:** [Count]
- **Quality:** [Assessment]
- **Clarity:** [Assessment]
- **Relevance:** [Assessment]

**Issues:**
- **[Diagram #1]**
  - **Location:** [Section]
  - **Issue:** [Description]
  - **Suggested Fix:** [Recommendation]

**Missing Diagrams:**
- **[Concept that needs diagram]**
  - **Suggested Type:** [Flowchart / Tree / Graph / etc.]
  - **Purpose:** [What it should illustrate]

### Tables
- **Number of Tables:** [Count]
- **Formatting:** [Assessment]
- **Clarity:** [Assessment]
- **Relevance:** [Assessment]

**Issues:**
- **[Table #1]**
  - **Location:** [Section]
  - **Issue:** [Description]
  - **Suggested Fix:** [Recommendation]

**Missing Tables:**
- **[Data that should be in table]**
  - **Suggested Format:** [Outline]

### Code Examples
- **Number of Code Blocks:** [Count]
- **Syntax Highlighting:** [Assessment]
- **Comments:** [Assessment]
- **Clarity:** [Assessment]

**Issues:**
- **[Code Block #1]**
  - **Location:** [Section]
  - **Issue:** [Description]
  - **Suggested Fix:** [Recommendation]

### Visual Integration
- [ ] All diagrams referenced in text
- [ ] All tables explained in text
- [ ] Visuals support understanding
- [ ] Alt text provided for accessibility
- [ ] Visuals don't duplicate text unnecessarily

---

## Structure Review

### Frontmatter
- [ ] Layout specified correctly
- [ ] Title is descriptive and SEO-friendly
- [ ] Date matches lecture date
- [ ] Categories appropriate
- [ ] Tags relevant and sufficient (5 tags)
- [ ] Excerpt is compelling (50-100 words)
- [ ] Reading time accurate
- [ ] Course specified correctly

**Issues:**
- **[Issue]:** [Description and fix]

### Heading Hierarchy
- [ ] Single H1 (title)
- [ ] H2 for main sections
- [ ] H3 for subsections
- [ ] H4 for sub-subsections (if needed)
- [ ] Logical nesting

**Issues:**
- **[Issue]:** [Description and fix]

### Section Organization
- [ ] Introduction present and effective
- [ ] Main content well-organized
- [ ] Exercises section present
- [ ] External resources section present
- [ ] Conclusion synthesizes key points
- [ ] Sections appropriately sized

**Issues:**
- **[Issue]:** [Description and fix]

---

## Strengths

[What works well in this draft]

1. **[Strength #1]**
   - **Description:** [Why this is effective]
   - **Impact:** [How it enhances the post]

2. **[Strength #2]**
   - **Description:** [Why this is effective]
   - **Impact:** [How it enhances the post]

3. **[Strength #3]**
   - **Description:** [Why this is effective]
   - **Impact:** [How it enhances the post]

---

## Priority Actions

### High Priority (Must Fix)
1. **[Action #1]**
   - **Reason:** [Why critical]
   - **Effort:** [Estimated time/complexity]

2. **[Action #2]**
   - **Reason:** [Why critical]
   - **Effort:** [Estimated time/complexity]

### Medium Priority (Should Fix)
1. **[Action #1]**
   - **Reason:** [Why important]
   - **Effort:** [Estimated time/complexity]

2. **[Action #2]**
   - **Reason:** [Why important]
   - **Effort:** [Estimated time/complexity]

### Low Priority (Nice to Have)
1. **[Action #1]**
   - **Reason:** [Enhancement]
   - **Effort:** [Estimated time/complexity]

---

## Specific Feedback by Section

### Introduction
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

### Section: [Section Name]
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

### Section: [Section Name]
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

### Exercises Section
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

### External Resources
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

### Conclusion
**Assessment:** [Good / Needs Improvement]
**Feedback:** [Specific comments]

---

## Comparison with Source Materials

### Alignment with Lecture Content
- [ ] All main topics from lecture covered
- [ ] Examples match lecture examples
- [ ] Definitions align with lecture definitions
- [ ] Mathematical notation consistent

**Discrepancies:**
- **[Discrepancy #1]**
  - **In Post:** [What post says]
  - **In Lecture:** [What lecture says]
  - **Resolution:** [Recommendation]

### Textbook References
- [ ] References accurate
- [ ] Page numbers correct (if specified)
- [ ] Concepts properly attributed

**Issues:**
- **[Issue]:** [Description]

---

## Final Recommendations

**For Next Revision:**
1. [Primary focus area]
2. [Secondary focus area]
3. [Tertiary focus area]

**Estimated Revision Time:** [X] hours

**Confidence Level:** [High / Medium / Low] that issues can be resolved

**Additional Notes:**
[Any other observations, suggestions, or concerns]

---

## Approval Status

- [ ] **Approve** - Post is ready for publication
- [ ] **Approve with Minor Changes** - Post is good, but should address minor issues
- [ ] **Needs Major Revision** - Significant issues must be addressed
- [ ] **Reject** - Post does not meet quality standards

**Signature:** [Agent Name]
**Date:** [YYYY-MM-DD]
