...
1. Identity Element Rejection: Implementations must explicitly reject the point at infinity (O). Failure to do so allows fixed-point exploits. Furthermore, protocols like Tendermint failed by not including participant identities in the Key Derivation Function (KDF), allowing attackers to reuse signatures across sessions.
