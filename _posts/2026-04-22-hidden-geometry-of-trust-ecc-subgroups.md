...
The set of points $(x, y)$ satisfying this equation, alongside a specific Point at Infinity ($\mathcal{O}$), forms an Abelian group. Geometrically, $\mathcal{O}$ serves as the identity element ($P + \mathcal{O} = P$). In a finite field $GF(p)$, every point $P = (x, y)$ has a unique additive inverse $-P$, calculated as $(x,\; p - y \pmod p)$. Geometrically, this is the vertical reflection of the point across the $x$-axis.
...
* Point Addition ($P \neq Q$): $s = \dfrac{y_2 - y_1}{x_2 - x_1} \pmod p$
* Point Doubling ($P = Q$): $s = \dfrac{3x_1^2 + a}{2y_1} \pmod p$
...
Every base point $G$ on a curve generates a cyclic subgroup. By iteratively performing scalar multiplication $(G, 2G, 3G, \dots)$, the sequence will eventually cycle back to the identity $\mathcal{O}$. The smallest positive integer $n$ such that $nG = \mathcal{O}$ is defined as the Order of the point.
...
Implementation security often diverges from mathematical theory. In embedded and IoT environments, developers may skip cofactor validation—verifying that a public key $Q$ satisfies $nQ = \mathcal{O}$—to conserve processing cycles. This creates a vulnerability to Small Subgroup Confinement Attacks.

If an attacker provides a point $P_{\text{small}}$ belonging to a subgroup of very small order, the resulting shared secret in an ECDH exchange becomes confined to that small subgroup. By observing the truncated output, an attacker can solve the discrete logarithm within that tiny space.
...
* Calculate the slope $s$: $s = \dfrac{3(11)^2 + 1}{2(3)} \pmod{23} = \dfrac{364}{6} \pmod{23}$.
...
* $s = \dfrac{3 - 0}{11 - 4} = \dfrac{3}{7} \pmod{23}$.
...
* Since $3P$ is the vertical reflection of $P$ ($P + (-P) = \mathcal{O}$), $4P = \mathcal{O}$.

Conclusion: The subgroup order $n$ for point $P(11, 3)$ is 4. Given the curve order $N=28$, the cofactor is: $h = \dfrac{28}{4} = 7$.
